<template lang="pug">
  div(style="position: relative")
    svg#color(@click="open" :style="colorStyle" viewBox="-21 -21 683 682")
      path(fill="#333"
...
<script lang="coffee">
  export default
    props: ['name', 'title', 'color', 'colorStyle', 'target']

    methods:
      open: ->
        pallete = @$parent.$parent.$parent.pallete
        ...
        if @target
             pallete.target = @target
        else pallete.target = @title
</script>

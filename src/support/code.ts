...
$('#about, #about button').hover(() => {
  mode > 0 ? $('#about button').css({ color: '#eee', background: '#222' })
           : $('#about button').css({ color: '#222', background: '#eee' })
}, () => {
  $('body').css('background-color') === 'rgb(255, 255, 255)' ? mode = 0 : mode = 1
  
  mode > 0 ? $('#about button').css({ color: '#222', background: '#eee' })
           : $('#about button').css({ color: '#eee', background: '#222' })
})
